...
-- Grant access to sales analytics only to admins and organizers
CREATE POLICY "Admins and organizers can view sales analytics" ON public.sales_analytics
FOR SELECT USING (
  has_role(auth.uid(), 'admin'::app_role) OR
  auth.uid() IN (SELECT user_id FROM public.organizers WHERE status = 'approved')
);